...
bool drawing_save_dxf(Drawing *drawing, String filename) {
    ...
    fprintf(file, "0\nSECTION\n2\nENTITIES\n");
    real mm_to_inches = 1.0f / 25.4f;
    for (Entity *entity = entities->array; entity < &entities->array[entities->length]; ++entity) {
        if (entity->type == EntityType::Line) {
            fprintf(file, "0\nLINE\n");
            fprintf(file, "8\n0\n");  // Layer
            fprintf(file, "62\n%d\n", entity->color_code);
            fprintf(file, "10\n%.6f\n", entity->line.start.x * mm_to_inches);
            fprintf(file, "20\n%.6f\n", entity->line.start.y * mm_to_inches);
            fprintf(file, "30\n0.0\n");  // Z coordinate (2D)
            fprintf(file, "11\n%.6f\n", entity->line.end.x * mm_to_inches);
            fprintf(file, "21\n%.6f\n", entity->line.end.y * mm_to_inches);
            fprintf(file, "31\n0.0\n");  // Z coordinate (2D)
        } else if (entity->type == EntityType::Arc) {
            fprintf(file, "0\nARC\n");
            fprintf(file, "8\n0\n");  // Layer
            fprintf(file, "62\n%d\n", entity->color_code);
            fprintf(file, "10\n%.6f\n", entity->arc.center.x * mm_to_inches);
            fprintf(file, "20\n%.6f\n", entity->arc.center.y * mm_to_inches);
            fprintf(file, "30\n0.0\n");  // Z coordinate (2D)
            fprintf(file, "40\n%.6f\n", entity->arc.radius * mm_to_inches);
