...
// XXXX: cooldown on bbpr restore so less flickery 
...
void conversation_draw() {
    ...
    { // draw 2D draw 2d draw
        ...
        {
            if (!other.hide_grid_2D) { // grid 2D grid 2d grid
                        ...
                        for_(pass, 3) {
                            ...
                            if (pass == DRAW2D_PASS_Mouse) {
                                if (1
                                    ...
                                    _for_each_entity_ {
                                        if (state_Xsel_command_is_(Select) && entity->is_selected) continue;
                                        if (state_Xsel_command_is_(Deselect) && !entity->is_selected) continue;
                                        bbox2 window = {
                                    ...
                                    if (other.hot_pane == Pane::Drawing) { // highlight (TODO: for 3D face as well)
                            ...
                            { // crosshairs
                                ...
                                if (not_drawing_on_top_of_system_cursor) {
                                    ...
                                    chowder_set_size(2.0f);
                                    real r = 0.9f * funky_OpenGL_factor;
    ...
    { // 3D draw 3D 3d draw 3d
        ...
        if (!other.hide_grid_3D) { // grid 3D grid 3d grid
            ...
            eso_color(CLAMPED_LINEAR_REMAP(f, 1.0f, 0.0f, pallete_3D->grid, pallete_3D->background));
            eso_size(1.0f);
            if (f > 0.0) {
        ...
        if (feature_plane->is_active) { // selection 2d selection 2D selection tube tubes slice slices stack stacks wire wireframe wires frame (FORNOW: ew) cage cageit pipe pipes
            ;
            // FORNOW
            ...
            vec3 target_preview_tubes_color = (0) ? V3(0)
                : (moving_stuff) ? pallete_3D->sketch_color_default // ??
                : (adding) ? pallete_3D->sketch_color_add
                : (cutting) ? pallete_3D->sketch_color_remove
                : pallete_3D->sketch_color_default;
