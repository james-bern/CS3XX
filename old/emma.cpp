...
int main() {
{
        ...
        { // triangle_tuples
            ...
            mesh.triangle_tuples = (uint3 *) calloc(mesh.num_triangles, sizeof(uint3));
            uint currTriangle = 0;
            // sides
            for (uint i = 0; i < mesh.num_vertices-4; i++) {
                mesh.triangle_tuples[currTriangle++] = { i, i+3, i+4 };
                mesh.triangle_tuples[currTriangle++] = { i, i+1, i+4 };
            }
            // ends
            mesh.triangle_tuples[currTriangle++] = { 0, 1, 3 };
            mesh.triangle_tuples[currTriangle++] = { 1, 2, 3 };
            mesh.triangle_tuples[currTriangle++] = { mesh.num_vertices-3, mesh.num_vertices-2, mesh.num_vertices-1 };
            mesh.triangle_tuples[currTriangle++] = { mesh.num_vertices-4, mesh.num_vertices-3, mesh.num_vertices-1 };
    ...
    while (begin_frame(&camera)) {
        ...
        { // render triangle mesh
            ...
            for_(triangle_index, mesh.num_triangles) {
                for_(d, 3) {
                    uint vertex_index = mesh.triangle_tuples[triangle_index][d];
                    vec3 color = {};
